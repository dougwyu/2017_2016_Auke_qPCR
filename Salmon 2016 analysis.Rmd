...
```{r}
# keep only one set of qPCRs per sampleID and remove remaining rows that have no CT value
sockeye_qpcr_filtered <- sockeye_qpcr %>% filter(CT_omit == "no_omit") %>% filter(CT != "NA")

# OR keep both sets of qPCRs per sampleID and remove remaining rows that have no CT value
...
# sockeye_qpcr_summary <- sockeye_qpcr_summary %>% select("Date", "SampleID", "Plate_ID", "SampleNumber", "SampleReplicate", "updownstream", "CT_mean", "CT_sd", "Qty_ng_per_ul_mean", "Qty_ng_per_ul_sd", "n")
