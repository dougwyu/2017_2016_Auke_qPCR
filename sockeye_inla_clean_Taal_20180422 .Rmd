...
sockeye_qpcr_env <- sockeye_qpcr_env %>% dplyr::mutate(Q_corr_qpcr.lag = lag(Q_corr_qpcr))
sockeye_qpcr_env <- sockeye_qpcr_env %>% dplyr::mutate(Q_corr_qpcr.lead = lead(Q_corr_qpcr))
...
# using doug's dataset, from the code above
...
#flowDNA vs count poisson
flowDNAvsCount=glm(SockeyeTotalCount~Q_corr_qpcr+Avg_Temp,data=dd.qpcr.env.adults.rm)
summary(flowDNAvsCount)
plot(dd.qpcr.env.adults.rm$Date,dd.qpcr.env.adults.rm$SockeyeTotalCount,type='l')
...
#count_t vs DNA
CountvsDNA=lm(Q_corr_qpcr[2:47]~SockeyeTotalCount[1:46]+SockeyeTotalCount[2:47],data=dd.qpcr.env.adults.rm)
summary(CountvsDNA)
